import { hex } from '@owf/identity-common'
import { X509Certificate } from '@peculiar/x509'
import { describe, expect, test } from 'vitest'
import {
  CoseKey,
  DeviceKey,
  DeviceRequest,
  DocRequest,
  Issuer,
  ItemsRequest,
  SessionTranscript,
  SignatureAlgorithm,
} from '../../src'
import { DeviceResponse } from '../../src/mdoc/models/device-response'
import {
  DEVICE_JWK_PRIVATE,
  DEVICE_JWK_PUBLIC,
  ISSUER_CERTIFICATE,
  ISSUER_PRIVATE_KEY_JWK,
  SECOND_DEVICE_JWK_PRIVATE,
  SECOND_DEVICE_JWK_PUBLIC,
} from '../config'
import { mdocContext } from '../context'

const encodedDeviceResponse =
  ''

describe('device response', () => {
  test('parse', () => {
    const deviceResponse = DeviceResponse.decode(hex.decode(encodedDeviceResponse))

    expect(deviceResponse.status).toStrictEqual(0)
    expect(deviceResponse.version).toStrictEqual('1.0')
    expect(deviceResponse.documentErrors).toBeUndefined()
    expect(deviceResponse.documents?.length).toStrictEqual(1)

    expect(deviceResponse.documents?.[0].docType).toStrictEqual('org.iso.18013.5.1.mDL')
    expect(deviceResponse.documents?.[0].issuerSigned.issuerAuth.jwaAlgorithm).toStrictEqual('ES256')

    expect(deviceResponse.documents?.[0].deviceSigned.deviceAuth.deviceMac?.jwaAlgorithm).toStrictEqual('HS256')
  })
})

const signed = new Date()
const validFrom = new Date(signed.getTime() - 60_000)
const validUntil = new Date(signed.getTime() + 365 * 24 * 60 * 60 * 1000)

const mdlDocType = 'org.iso.18013.5.1.mDL'
const photoIdDocType = 'org.iso.23220.photoid.1'
const namespace = 'org.iso.18013.5.1'
const requestedNamespaces = { [namespace]: { family_name: true } }

const issuerCertificate = new Uint8Array(new X509Certificate(ISSUER_CERTIFICATE).rawData)
const trustedCertificates = [{ issuance: [issuerCertificate] }]

async function issue(docType: string, deviceKeyJwk: Record<string, unknown>) {
  const issuer = new Issuer(docType, mdocContext)
  issuer.addIssuerNamespace(namespace, { family_name: 'Doe', given_name: 'John' })

  return await issuer.sign({
    signingKey: CoseKey.fromJwk(ISSUER_PRIVATE_KEY_JWK),
    certificates: [issuerCertificate],
    algorithm: SignatureAlgorithm.ES256,
    digestAlgorithm: 'SHA-256',
    deviceKeyInfo: { deviceKey: DeviceKey.fromJwk(deviceKeyJwk) },
    validityInfo: { signed, validFrom, validUntil },
  })
}

function deviceRequestFor(...docTypes: Array<string>) {
  return DeviceRequest.create({
    docRequests: docTypes.map((docType) =>
      DocRequest.create({ itemsRequest: ItemsRequest.create({ docType, namespaces: requestedNamespaces }) })
    ),
  })
}

describe('device response createWithDeviceRequest', () => {
  test('answers a multi document request, each document with its own device key', async () => {
    const mdl = await issue(mdlDocType, DEVICE_JWK_PUBLIC)
    const photoId = await issue(photoIdDocType, SECOND_DEVICE_JWK_PUBLIC)

    const sessionTranscript = await SessionTranscript.forOid4Vp(
      { clientId: 'my-client-id', responseUri: 'my-response-uri.com', nonce: 'my-random-nonce' },
      mdocContext
    )

    const deviceResponse = await DeviceResponse.createWithDeviceRequest(
      {
        deviceRequest: deviceRequestFor(mdlDocType, photoIdDocType),
        sessionTranscript,
        documents: [
          { issuerSigned: mdl, docRequestIndex: 0, signature: { signingKey: CoseKey.fromJwk(DEVICE_JWK_PRIVATE) } },
          {
            issuerSigned: photoId,
            docRequestIndex: 1,
            signature: { signingKey: CoseKey.fromJwk(SECOND_DEVICE_JWK_PRIVATE) },
          },
        ],
      },
      mdocContext
    )

    expect(deviceResponse.documents?.map((document) => document.docType)).toStrictEqual([mdlDocType, photoIdDocType])

    // Both device signatures verify against the device key of their own document.
    await expect(deviceResponse.verify({ sessionTranscript, trustedCertificates }, mdocContext)).resolves.toHaveLength(
      2
    )
  })

  test('answers only the documents that were provided', async () => {
    const mdl = await issue(mdlDocType, DEVICE_JWK_PUBLIC)

    const deviceResponse = await DeviceResponse.createWithDeviceRequest(
      {
        deviceRequest: deviceRequestFor(mdlDocType, photoIdDocType),
        sessionTranscript: await SessionTranscript.forOid4Vp(
          { clientId: 'my-client-id', responseUri: 'my-response-uri.com', nonce: 'my-random-nonce' },
          mdocContext
        ),
        documents: [
          { issuerSigned: mdl, docRequestIndex: 0, signature: { signingKey: CoseKey.fromJwk(DEVICE_JWK_PRIVATE) } },
        ],
      },
      mdocContext
    )

    expect(deviceResponse.documents?.map((document) => document.docType)).toStrictEqual([mdlDocType])
  })

  test('the docRequestIndex selects between doc requests for the same doc type', async () => {
    const mdl = await issue(mdlDocType, DEVICE_JWK_PUBLIC)

    const deviceResponse = await DeviceResponse.createWithDeviceRequest(
      {
        deviceRequest: deviceRequestFor(mdlDocType, mdlDocType),
        sessionTranscript: await SessionTranscript.forOid4Vp(
          { clientId: 'my-client-id', responseUri: 'my-response-uri.com', nonce: 'my-random-nonce' },
          mdocContext
        ),
        documents: [
          { issuerSigned: mdl, docRequestIndex: 1, signature: { signingKey: CoseKey.fromJwk(DEVICE_JWK_PRIVATE) } },
        ],
      },
      mdocContext
    )

    expect(deviceResponse.documents).toHaveLength(1)
  })

  test('throws when the docRequestIndex is out of range', async () => {
    const mdl = await issue(mdlDocType, DEVICE_JWK_PUBLIC)

    await expect(
      DeviceResponse.createWithDeviceRequest(
        {
          deviceRequest: deviceRequestFor(mdlDocType),
          sessionTranscript: await SessionTranscript.forOid4Vp(
            { clientId: 'my-client-id', responseUri: 'my-response-uri.com', nonce: 'my-random-nonce' },
            mdocContext
          ),
          documents: [
            { issuerSigned: mdl, docRequestIndex: 3, signature: { signingKey: CoseKey.fromJwk(DEVICE_JWK_PRIVATE) } },
          ],
        },
        mdocContext
      )
    ).rejects.toThrow('No doc request found at index 3')
  })
})
